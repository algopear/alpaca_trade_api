...
def get_credentials(oauth: str = None) -> Credentials:
    oauth = oauth or os.environ.get('OAUTH_TOKEN')

    if oauth is None:
        raise ValueError('Auth Token must be given to access Alpaca trade API',
                         ' (env: OAUTH_TOKEN)')
    return oauth
...
#     try:
#         alpaca_key = get_credentials(alpaca_key)
#     except ValueError:
#         pass
    key_id = os.environ.get('POLYGON_ID') or alpaca_key
    if key_id is None:
