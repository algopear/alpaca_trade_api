...
class _StreamConn(object):
    def __init__(self, oauth: str,base_url: URL):
        self._oauth = oauth
...
class StreamConn(object):
    ...
    def __init__(
        ...
        self._oauth = get_credentials(oauth)
        ...
        self.trading_ws = _StreamConn(self._base_url,
                                      self._oauth)
        ...
        if self._data_stream == 'polygon':
            ...
            self.data_ws = _StreamConn(self._data_url,
                                       self._oauth)
            self._data_prefixes = (
                ('Q.', 'T.', 'AM.', 'alpacadatav1/'))
    ...
    async def close(self, renew):
        ...
        if self.data_ws is not None:
            ...
            self.data_ws = None
        if renew:
            self.trading_ws = _StreamConn(self._oauth,
                                          self._base_url)
            if self._data_stream == 'polygon':
                self.data_ws = polygon.StreamConn(
                ...
                self.data_ws = _StreamConn(self._oauth,
                                           self._data_url)
